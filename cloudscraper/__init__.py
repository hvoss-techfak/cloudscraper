...
class CloudScraper(Session):
    ...
    def request(self, method, url, *args, **kwargs):
        top_level = self._enter_request_context()
        try:
            ...
            self._exit_request_context(top_level)
    ...
    def _enter_request_context(self):
        """
        Reserve a concurrency slot only for the outer most request.
    ...
    def _exit_request_context(self, top_level: bool):
        """
        Release the concurrency slot that _enter_request_context reserved.
        Always executed in a finally-block so the counter can never be left
        out of sync – even when an exception is raised or the function
        returns early.
        """
        self._request_depth -= 1
